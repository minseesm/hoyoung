...
def go_train() :
    ...
    select = input("[Select Model]\n\n [1] Linear \n [2] Linear Custom nodes \n\n Select : ")
    device, model = train_model(select=select)

    trained_model = training(epochs, batch_size, learning_rate, model, device)
    if select == '1':
        torch.save(trained_model, f"{os.path.dirname(__file__)}/trained_model/Linear_{epochs}_{learning_rate}.pth")
    elif select == '2':
        torch.save(trained_model, f"{os.path.dirname(__file__)}/trained_model/Linear_custom_convT_{epochs}_{learning_rate}.pth")
